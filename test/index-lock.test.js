...
describe('index lock', () => {
  test('a second run on the same project is refused, not allowed to race', () => {
    const root = mkdtempSync(join(tmpdir(), 'codelens-lock-'));
    try {
      mkdirSync(join(root, '.codelens'));
      ...
      assert.throws(() => acquireIndexLock(root), (err) => err.code === 'CODELENS_LOCKED');
      release();
  ...
  test('takes over a lock whose process is gone', () => {
    ...
    try {
      ...
      writeFileSync(join(root, '.codelens', 'index.lock'), '999999');
      const release = acquireIndexLock(root);
      assert.equal(readFileSync(join(root, '.codelens', 'index.lock'), 'utf8'), String(process.pid));
      release();
      assert.equal(existsSync(join(root, '.codelens', 'index.lock')), false);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  test('does not lock, or litter, a tree with no index directory', () => {
    ...
    try {
      acquireIndexLock(root)();
      assert.equal(existsSync(join(root, '.codelens')), false);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
